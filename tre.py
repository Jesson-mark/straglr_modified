...
class TREFinder:
    def __init__(self, bam, genome_fasta, reads_fasta=None, check_split_alignments=True,
        ...
        self.tmp_prefix = os.path.basename(self.out_prefix)
    ...
    def perform_trf(self, seqs):
        trf_fasta = create_tmp_file(seqs, self.tmp_prefix)
        if self.debug:
            logging.info('trf input file is {}'.format(trf_fasta))
        self.tmp_files.add(trf_fasta)
        
        logging.info("Begin to run trf for %s"%(trf_fasta))
        output = self.run_trf(trf_fasta)
        logging.info("Begin to parse result of trf for %s"%(trf_fasta))
    ...
    def run_blastn(self, query_fa, target_fa, word_size):
        query_file = create_tmp_file(query_fa, self.tmp_prefix)
        target_file = create_tmp_file(target_fa, self.tmp_prefix)
        blastn_out = create_tmp_file('', self.tmp_prefix)
    ...
    def extract_alleles_trf(self, trf_input, repeat_seqs, flank, clipped, bam, strands, patterns, reads_fasta, too_far_from_read_end=200):
        logging.info("Begin to perform trf")
        results = self.perform_trf(trf_input)
        logging.info("Begin to run find_similar_long_patterns_gt")
        same_pats = self.find_similar_long_patterns_gt(results, patterns)

        # group by locus
        alleles = defaultdict(dict)

        logging.info("Begin to group alleles by locus")
        for seq in results.keys():
            cols = seq.split(':')
            if len(cols) < 7:
                if self.debug:
    ...
    def get_alleles(self, loci, reads_fasta=[], closeness_to_end=50):
        ...
        logging.info("Begin to construct trf input")
        for locus in loci:
        ...
        variants = []
        if trf_input:
            logging.info("Begin to extract alleles from trf input")
        ...
        logging.info("Begin to genotype vntrs by GMM")
    ...
    def genotype(self, loci_bed):
        ...
        logging.info("Total %s loci are loaded"%(len(loci)))
